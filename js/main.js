...
function layDuLieuDauVao(isAdd){
    ...
    // if (_maSV.trim() === ""){
    //     //thông báo lỗi
    //     getEle("maError").innerHTML = "(*) Vui lòng nhập mã sinh viên";

    //     //add class overwrite tương tự innerHTML
    //     getEle("maError").className = "alert alert-danger";

    //     // //thêm class vào class đã có sẵn
    //     // getEle("maEror").classList.add("alert");

    // isValid = false;

    // } else {
    //     getEle("maError").innerHTML = "";
    //     getEle("maError").className = "";

    //     // //xoá class vào class đã có sẵn
    //     // getEle("maEror").classList.add("");
    ...
    isValid &= validation.kiemTraRong(
        ...
        && validation.kiemTraDoDaiKyTu(
            _maSV, 
            "maError", 
            "(*) Mã sinh viên từ 4 - 10 ký tự", 4, 10)
...
function getLocalStorage(){
    ...
    //Làm thêm DOM tới các thẻ div show error
    });
